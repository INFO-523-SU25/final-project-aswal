...
Summary: The dataset reveals average daily screen time of 5 hours, with a standard deviation of 1.5 hours. Sleep duration averages around 7 hours, indicating potential sleep deprivation among heavy smartphone users.
...
```
Observation: The distribution of daily screen time shows a right-skewed pattern, suggesting a significant portion of teenagers spend less time on their phones, while a smaller group exhibits higher usage.
...
plt.show()
```
Insight: Strong negative correlation between sleep duration and daily screen time indicates that increased screen time may be associated with reduced sleep.
...
Findings: No significant difference in daily screen time between genders, suggesting that smartphone usage patterns are similar across genders.
...
plt.show()
```
Observation: A negative trend is evident, where increased phone usage correlates with decreased sleep duration.
...
plt.show()
```
Result: Approximately 5% of the dataset exhibits behaviors indicative of smartphone addiction.
