...
\`\`\`{python} #\| label: load-pkgs #\| message: false #\| echo: false
...
data = pd.read_csv("data/teen_phone_addiction.csv") data.head()

#\| label: hist-usage #\| fig-cap: "Distribution of Daily Screen Time" #\| fig-align: center #\| echo: false

plt.figure(figsize=(6,4)) sns.histplot(data\['Screen Time (hours/day)'\], bins=20, kde=True, color='teal') plt.xlabel("Daily Screen Time (Hours)") plt.ylabel("Number of Teens") plt.title("Distribution of Teen Screen Time") plt.show()

#\| label: age-vs-usage #\| fig-cap: "Screen Time by Age" #\| fig-align: center #\| echo: false

plt.figure(figsize=(6,4)) sns.boxplot(x='Age', y='Screen Time (hours/day)', data=data, palette='coolwarm') plt.title("Teen Screen Time by Age") plt.show()
...
\`\`\`bash teen-smartphone-addiction/ │ ├── data/ │ ├── raw/ \# Original dataset from Kaggle │ ├── processed/ \# Cleaned datasets │ └── codebook.md \# Data dictionary │ ├── notebooks/ │ ├── 01_eda.ipynb │ ├── 02_isolation_forest.ipynb │ ├── 03_one_class_svm.ipynb │ └── 04_autoencoder.ipynb │ ├── src/ │ ├── preprocessing.py │ ├── anomaly_models.py │ ├── evaluation.py │ └── utils.py │ ├── reports/ │ ├── proposal.qmd │ ├── final_report.qmd │ └── presentation.qmd │ ├── dashboards/ │ └── app.py \# Optional Streamlit dashboard │ ├── README.md └── requirements.txt